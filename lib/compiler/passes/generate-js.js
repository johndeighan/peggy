...
const {ByteCodeWriter} = require('./ByteCodeWriter.js');
const {OpDumper} = require('./OpDumper.js');

let byteCodeWriter, opDumper;
...
function generateJS(ast, options) {
  ...
  let {dumpAST, dumpOps, writeByteCodes,
    allowedStartRules, genMarkers,
    } = options;

  // --- when set to true, unit tests fail, I think because source map changes
  genMarkers = false;
  ...
  if (writeByteCodes) {
    byteCodeWriter = new ByteCodeWriter(ast, {detailed: false});
    }
  if (dumpOps) {
    opDumper = new OpDumper();
    }
