...
const { dclone } = require('./passes/vllu.js');

var alreadyDumped = false;
var clonedDS = undefined;
...
class Stack {
  ...
  dumpOnce(label) {
    ...
    let before = (label.indexOf('before') == 0)
    let after = (label.indexOf('after') == 0)

    console.log(label);
    if (before) {
      console.dir(this);
      clonedDS = dclone(this);
      }
    else if (after) {
      let hDiffs = diff(clonedDS, this)
      console.dir(hDiffs);
      alreadyDumped = true;
      }
    else {
